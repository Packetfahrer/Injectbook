...
NSMutableArray *injectbookGenerateDylibList() {
	...
	for (NSString *plist in plists) {
		...
		for (NSString *entry in filter[@"Filter"][@"Bundles"]) {
			if (![entry isEqualToString:@"com.apple.UIKit"] && ![entry isEqualToString:@"com.facebook.Facebook"] && !CFBundleGetBundleWithIdentifier((CFStringRef)entry)) {
				// If not, skip it
				continue;
			}
		...
		if (!isInjected) {
			...
			for (NSString *process in filter[@"Filter"][@"Executables"]) {
				if ([process isEqualToString:@"Facebook"]) {
					[dylibsToInject addObject:dylibPath];
					isInjected = YES;
					break;
				}
...
- (NSDictionary *)environmentVariables {
	return [self.bundleIdentifier isEqualToString:@"com.facebook.Facebook"] ? overridedEnv(%orig) : %orig;
}
